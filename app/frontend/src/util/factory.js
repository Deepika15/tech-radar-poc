...
const plotRadar = function (blips, currentRadarName, alternativeRadars) {
  ...
  _.each(blips, function (blip) {
    ...
    quadrants[blip.quadrant].add(
      new Blip(blip._id, blip.updatedAt, blip.name, ringMap[blip.ring], blip.isNew.toLowerCase() === 'true', blip.isDeleted, blip.topic, blip.description),
    )
  })
...
const GoogleSheet = function (sheetReference, sheetName) {
  ...
  function createBlipsForProtectedSheet(documentTitle, values, sheetNames) {
    ...
    plotRadar(blips, sheetName, sheetNames)
  }
...
const GoogleSheetInput = function () {
  ...
  self.build = function () {
      ...
      sheet = JSONFile(SERVER_URL + "/blip/viewAllBlipsNoHistory/")
      sheet.init().build()
  }
...
function plotErrorMessage(exception) {
  if (config.featureToggles.UIRefresh2022) {
    ...
    d3.select('body').attr('onload', 'script();')
    setDocumentTitle()
